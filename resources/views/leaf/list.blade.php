    ...
    <!-- <section class="app-user-list">
        <div class="card">
            <div class="card-body border-bottom">
                <h4 class="card-title">{{ translate('common.leaf') }}</h4>
                <div class="dt-action-buttons text-end">
                    <a class="btn btn-primary" href="{{ route('leaf.add') }}">{{ translate('common.add_leaf') }}</a>
                </div>
            </div> -->
            <x-container title="leaf" buttonTitle="Add Leaf" buttonRoute="{{ route('leaf.add') }}">
            <div class="table-responsive pt-0">
                <!-- <table id="leafTable" class="table-bordered border-dark"> -->
                <table class="leafTable table table-bordered border-dark">
                    <thead class="x-table">
                        <tr>
...
<script>
    ...
    $(function() {
        var table = $('.leafTable').DataTable({
            processing: false,
            serverSide: true,
            ajax: "{{ route('leaf') }}", // AJAX request to the controller
            columns: [
            ...
            language: {
                "emptyTable": "No matching records found" // Custom message for empty table
            }
        });
    });
</script>
