...
@section('content')
                            ...
                            <img height="70" width="400" class="logo"
                                ...
                                <div class="text-right" style="padding: 7px 12px 10px 0;float: right;">
                                    ...
                                    <h5 class="" style="font-size: 1rem; margin-bottom: 0">
                                        {{ translate('common.address') }} : {{ setting('address') }}
                                    </h5>
                                    <h5 class="" style="font-size: 0.9rem; margin-bottom: 0">
                                        {{ translate('common.Phone') }} : {{ setting('phone') }}
                                    </h5>
                                </div>
                                <table class="bs-0">
                                    <tbody>
    ...
    @foreach ($purchase_details as $purchase)
        @if ($purchase->qty > 0) <!-- Only display the row if qty > 0 -->
            <tr>
    ...
    <!-- Only show the totals if at least one row with qty > 0 was displayed -->
    @if ($purchase_details->where('qty', '>', 0)->count() > 0)
        <tr>
    ...
    <thead>
        <tr>
            ...
            <th>{{ translate('common.Buy Price Per Unit') }}</th>
            <th>{{ translate('common.Quantity') }}</th>
            <th>{{ translate('common.date of return') }}</th>
            <th>{{ translate('common.Total') }}</th>
        </tr>
    </thead>
    ...
    <tbody>
        @foreach ($returns as $return)
            ...
            @endphp
            @if ($return->quantity !== null && $return->quantity > 0)
            <tr>
                <td>{{ $medicine->name }}</td>
                <!-- <td>{{ $batch->name }}</td> -->
                ...
                <td>{{ number_format($batch->buy_price * $return->quantity, 2) }}</td>
            </tr>
            @endif
        @endforeach
